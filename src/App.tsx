  ...
  return (
    <div className="App">
          ...
          <div className="message" key={message.id}>
            <div className="message-header">
              ...
              <span
                ...
                onClick={() => chatApi.toggleLike(message)}>
                Like
              </span>
      ...
      <div className="write-message">
        <select
          onChange={event => chatApi.selectUser(event.target.value)}
          value={chatApi.currentUser?.id}>
          {chatApi.users?.map(user => (
            <option id={user.name} key={user.id} value={user.id}>
              {user.name}
            </option>
          ))}
        </select>
      ...
      {chatApi.selectedAuthor && (
        <div className="popup">
          <div
            className="close"
            onClick={() => chatApi.setSelectedAuthor(null)}>
            X
          </div>
      ...
      {chatApi.showMessageDetails && chatApi.selectedMessage && (
        <div className="popup">
          <div
            className="close"
            onClick={() => chatApi.setShowMessageDetails(false)}>
            X
          </div>
          <h2>Liked By:</h2>
          <ul>
            {chatApi.selectedMessage.likes!.map(userId => {
              ...
              return likedUser && <li key={likedUser.id}>{likedUser.name}</li>;
            })}
          </ul>
        </div>
      )}
